...
| Output relay close timing | Output relay may close only after the per-Output minimum guarantee is prepared for the EV (default `[50,75,75,50]` config = 125 kW; SPEC §11) |
...
- **FR-14 path** (`step_planner.plan_transition`, called by `evcs_core_adapter.generate_control_steps`): rebuild + diff — build a Present engine and a Target engine, diff their states, emit a `ControlStepSequence` whose ordering obeys SPEC §11 (per-Output minimum guarantee — default 125 kW — before closing Output, Output stays Closed mid-charge, inter-Group/bridge close before Output on arrival, open before Output on departure). Determinism: same `(system_config, present, target)` → identical sequence. P95 ≤ 5 s.
